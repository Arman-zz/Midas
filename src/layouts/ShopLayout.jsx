...
const items = [
  ...
  { id: 'confirmations', label: 'Payment Confirmations', icon: 'check', count: 7 },
  { id: 'commissions', label: 'Commission Statements', icon: 'percent' },
  { id: 'reports', label: 'Reports', icon: 'list' },
]
export default function ShopLayout({ children }) {
  const active = readRoute().view || 'dashboard'
  return (
    <WorkspaceLayout
      role="shop"
      ...
      items={items}
      active={active}
      title={items.find((i) => i.id === active)?.label || 'Dashboard'}
      subtitle="Diamond Plaza — Dhanmondi"
    >
      {children}
    </WorkspaceLayout>
  )
